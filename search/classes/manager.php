...
use local_externaldocument\controller\extdoc_typeselector;
...
class manager {
    ...
    public function search(\stdClass $formdata, $limit = 0) {
        ...
        if (!$contextinfo->everything && !$contextinfo->usercontexts) {
            ...
            if (isset($contextinfo->usercontexts)) {
                foreach ($types as $type) {
                    $contextinfo->usercontexts['local_externaldocument-' . strtolower($type)] = [1 => 1];
                }
            }
            //-------------------Fi patch
