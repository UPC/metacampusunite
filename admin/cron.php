...
if (!empty($CFG->cronclionly)) {
	...
	// @PATCH PERFORMANCE
	// PATCH MCU 004 Executar cron web en background
	//------ Afegit:
	if (empty($CFG->pathtophp)) {
		mtrace('pathtophp not configured!');
		exit;
	}
	$command = 'nice -n 4 '.$CFG->pathtophp.' '.$CFG->dirroot.'/admin/cli/cron.php';
	$command = 'nohup '.$command.' > '.$CFG->dataroot.'/lastcron 2>&1 & echo $!';
	exec($command);
